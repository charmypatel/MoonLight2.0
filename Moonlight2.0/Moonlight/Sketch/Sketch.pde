...
//Set up runs everything only one time, full screen makes the screen full screen, noSmooth is for making the lines of the wave jagged (removed )
void setup() {

  //Makes sketch full screen
  fullScreen();

  //Setting path for each image
...
void draw() { 
  ...
  //Drawing images to screen, setting its size, & centering it
  imageMode(CENTER);
  image(owl, 700, 500, 200, 350); 
...
void mousePressed() {
  ...
  //Checking to see which coordinates user clicks around, by comparing them you will know which image was clicked
  ...
  //Pause button
  if (mouseX > 80 && mouseX < 130 && mouseY > 15 && mouseY < 65) { 
    player.pause();
  } 
  //Shuffle button
